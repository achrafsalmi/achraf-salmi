...
<html>
    ...
    <%persImpli ps = new persImpli();
        String s = "SELECT * FROM `user` WHERE `id_group` ='9'";
        String s7 = null;
        ResultSet res = null;
        ResultSet res1 = null;
        ...
        int k = 0;
        PreparedStatement ps1 = conn1.getConnection().prepareStatement(s);
        res = ps1.executeQuery(s);
        int i = 0;
        String s1 = ("SELECT * FROM easybee where `Identifiant` in (SELECT `Identifiant` FROM easybee GROUP by `Identifiant` HAVING COUNT(*) < 3) AND `Téléphone_Fixe` like '' AND `Statut` LIKE 'Nouveau' AND `ReferenceTT` LIKE ''");
        ...
        <form method="post" action="affectation_tache">
            <table>
                ...
                <tr>
                    ...
                    <td><input type="checkbox" id="vehicle" name="vehicle<%=i%>" value="<%=res.getString("id")%>"/></td>
                    <td><label for="vehicle1"> <%=res.getString("nom")%></label></td>
                    <td>
                        <label>date debut</label>
                        <input type="date" name="date_debut" >
                        <label>date fin</label><input type="date" name="date_fin">
                    </td>
                    <td><input type="number" id="vehicle" name="nb_demande<%=i%>" value="" size="2" min="0" max="1000"/></td>
                </tr>
            ...
            <table id="example"  style="width:60%;font-size: 10px;font-weight: bold;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><%=res1.getString("id_traitement")%></td>
                        <td><%=res1.getString("id_agent")%></td>
                    </tr>
                    <%}%>
